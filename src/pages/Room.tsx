...
const Room = () => {
    ...
    const handleEnded = () => {
      ...
      if (isHost && queueRef.current.length > 0) {
        ...
        if (nextTrack && syncedClock.isReady()) {
          const targetTime = syncedClock.now() + 10000;
   ...
   */
  const scheduleTrackForPlayback = useCallback((trackIdx: number, delayMs: number = 10000) => {
    if (!isHost) return;
  ...
  const handleTogglePlay = useCallback(() => {
    ...
    if (!audioEl.paused) {
      ...
      scheduleTrackForPlayback(currentIndexRef.current, 10000);
    }
  ...
  const handleNext = useCallback(() => {
    ...
    // Schedule next track with synced clock (10s buffer for pre-fetch)
    scheduleTrackForPlayback(nextIdx, 10000);
  ...
  const handlePrevious = useCallback(() => {
    ...
    scheduleTrackForPlayback(prevIdx, 10000);
  }, [isHost, isShuffle, scheduleTrackForPlayback]);
  ...
  const handleTrackClick = useCallback((idx: number) => {
    ...
    scheduleTrackForPlayback(idx, 10000);
  }, [isHost, scheduleTrackForPlayback]);
